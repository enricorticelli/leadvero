...
Represents one deep-analysis execution for a specific lead, including config, lifecycle state, progress counters, and aggregated summary.

Evidence: `prisma/schema.prisma`, `src/server/jobs/lead-analysis.ts`, `src/app/api/leads/[id]/analyses/route.ts`.

## LeadAnalysisPage

Represents one page snapshot generated by a deep-analysis run, including technical SEO fields and normalized issue list.

Evidence: `prisma/schema.prisma`, `src/server/jobs/lead-analysis.ts`, `src/app/api/leads/[id]/analyses/[runId]/route.ts`.
